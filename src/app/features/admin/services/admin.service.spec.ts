...
describe('AdminService', () => {
  ...
  it('should confirm a full external payment', () => { service.confirmarPagamentoExterno(9, 'INTEGRAL', 'PIX').subscribe(); const req = http.expectOne('http://api.test/api/admin/encomendas/9/pagamentos/confirmacao-externa'); expect(req.request.method).toBe('POST'); expect(req.request.body).toEqual({ tipo: 'INTEGRAL', forma: 'PIX', origem: 'EXTERNO_MANUAL' }); req.flush({ id: 7 }); });
  ...
  it('should update store settings', () => { const body = { entregaHabilitada: false, chavePix: 'pix@teste.local', dadosDeposito: 'Banco Teste' }; service.atualizarConfiguracaoLoja(body).subscribe(); const req = http.expectOne('http://api.test/api/admin/configuracao-loja'); expect(req.request.method).toBe('PUT'); expect(req.request.body).toEqual(body); req.flush(body); });
