...
export class EncomendasAdminPage implements OnInit {
  ...
  private readonly transitions: Record<string, string[]> = { PAGAMENTO_INICIAL_CONFIRMADO: ['PRODUCAO_LIBERADA','CANCELADO'], PRODUCAO_LIBERADA: ['EM_PRODUCAO','CANCELADO'], EM_PRODUCAO: ['EM_ACABAMENTO','CANCELADO'], EM_ACABAMENTO: ['PRONTO_PARA_ENTREGA_RETIRADA','CANCELADO'], PRONTO_PARA_ENTREGA_RETIRADA: ['AGUARDANDO_PAGAMENTO_RESTANTE','CANCELADO'], AGUARDANDO_PAGAMENTO_RESTANTE: ['ENVIADO_OU_RETIRADO','CANCELADO'], ENVIADO_OU_RETIRADO: ['CONCLUIDO','CANCELADO'], AGUARDANDO_PAGAMENTO_INICIAL: ['CANCELADO'] };
  ...
  protected nextStatuses(order: Pedido): string[] { return this.transitions[order.statusEncomenda] ?? []; }
  protected confirmPayment(order: Pedido, tipo: string, forma: string): void { this.service.confirmarPagamentoExterno(order.id, tipo as 'SINAL' | 'INTEGRAL', forma as 'PIX' | 'DINHEIRO' | 'DEPOSITO').pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => { this.message.set(`Pagamento da encomenda #${order.id} confirmado. A produção já pode ser liberada.`); this.load(); }, error: () => this.message.set('Não foi possível confirmar o pagamento. Verifique os pagamentos pendentes da encomenda.') }); }
}
