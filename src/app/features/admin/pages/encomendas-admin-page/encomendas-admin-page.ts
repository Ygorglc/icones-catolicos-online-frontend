...
export class EncomendasAdminPage implements OnInit {
  ...
  private readonly transitions: Record<string, string[]> = { AGUARDANDO_PAGAMENTO_RESTANTE: ['ENVIADO_OU_RETIRADO','CANCELADO'], ENVIADO_OU_RETIRADO: ['ENTREGUE_E_CONCLUIDO','CANCELADO'], AGUARDANDO_PAGAMENTO_INICIAL: ['CANCELADO'] };
  ...
  protected nextStatuses(order: Pedido): string[] { if (order.statusEncomenda === 'EM_PRODUCAO') return order.statusFinanceiro === 'PAGO_INTEGRALMENTE' ? ['ENVIADO_OU_RETIRADO','CANCELADO'] : ['AGUARDANDO_PAGAMENTO_RESTANTE','CANCELADO']; return this.transitions[order.statusEncomenda] ?? []; }
  protected confirmPayment(order: Pedido, tipo: string, forma: string): void { this.service.confirmarPagamentoExterno(order.id, tipo as 'SINAL' | 'INTEGRAL', forma as 'PIX' | 'DINHEIRO' | 'DEPOSITO').pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => { this.message.set(`Pagamento da encomenda #${order.id} confirmado. A encomenda está em produção.`); this.load(); }, error: () => this.message.set('Não foi possível confirmar o pagamento. Verifique os pagamentos pendentes da encomenda.') }); }
}
