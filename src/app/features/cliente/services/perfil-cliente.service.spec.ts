...
describe('PerfilClienteService', () => {
  ...
  it('should update only the authenticated client profile', () => { const body = { nome: 'Maria Silva', telefone: '21999999999', cpf: null, endereco: 'Rio de Janeiro' }; service.atualizar(body).subscribe(); const request = http.expectOne('http://api.test/api/clientes/me'); expect(request.request.method).toBe('PUT'); expect(request.request.body).toEqual(body); request.flush({ nome: 'Maria Silva' }); });
});
